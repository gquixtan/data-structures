...
def sort_by_cohort(filename):
    ...
    for content in contents:
        content_list = content.split("|")
        if content[-1] == "Fall 2015":
            fall_15.append(content[0] + " " + content[1])
            # all_students.append(fall_15)

    for content in contents:
        contents_list = content.split("|")
        if content[-1] == "Summer 2016":
            summer_16.append(content[0] + " " + content[1])

    for content in contents:
        contents_list = content.split("|")
        if content[-1] == "Spring 2016":
            spring_16.append(content[0] + " " + content[1])
            # all_students.append(spring_16)

    for content in contents:
        contents_list = content("|")
        if content[-1] == "Winter 2016":
            winter_16.append(content[0] + " " + content[1])
            # all_students.append(winter_16)

    for content in contents:
        contents_list = content("|")
        if content[-1] == "G" and content[-1] == "I":
            ghosts.append(content[0] + " " + content[1])
            # all_students.append(ghosts)

    all_students = winter_16 + spring_16 +  summer_16 + fall_15 + ghost
    print all_students, "printttttME!"
